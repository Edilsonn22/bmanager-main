...
export default function Fornecedor() {
  ...
  const remover = async () => {
    const fornecedor = fornecedorExcluir;
    ...
    try {
      ...
      setFornecedorExcluir(null); setSucesso("Fornecedor excluído com sucesso.");
    } catch (error) {
  ...
  return <main className="h-screen min-w-0 flex-1 overflow-auto bg-gray-50 p-4 sm:p-6 md:p-7">
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="overflow-x-auto"><table className="w-full min-w-[760px]">
        ...
        <tbody className="divide-y divide-gray-200 text-sm">{visiveis.map((fornecedor) => <tr key={fornecedor.id} className="hover:bg-gray-50">
          ...
          {podeGerir && <td className="whitespace-nowrap px-6 py-3 text-right"><Link aria-label={`Editar ${fornecedor.nome}`} to={`/editarFornecedor/${fornecedor.id}`} className="inline-flex rounded-lg p-2 hover:bg-gray-100"><Edit2 className="h-4 w-4 text-gray-600" /></Link>{podeExcluir && <button type="button" aria-label={`Excluir ${fornecedor.nome}`} disabled={removendo === fornecedor.id} onClick={() => setFornecedorExcluir(fornecedor)} className="inline-flex rounded-lg p-2 hover:bg-red-50 disabled:opacity-50"><Trash2 className="h-4 w-4 text-red-600" /></button>}</td>}
        </tr>)}</tbody>
      </table></div>
      {loading && <p className="p-8 text-center text-gray-500">A carregar fornecedores...</p>}
      {!loading && !visiveis.length && <div className="p-10 text-center"><Truck className="mx-auto h-10 w-10 text-slate-300"/><p className="mt-3 font-semibold text-slate-700">{pesquisa ? "Nenhum fornecedor corresponde à pesquisa." : "Ainda não existem fornecedores."}</p>{podeGerir && !pesquisa && <Link to="/adicionarFornecedor" className="mt-3 inline-flex font-semibold text-indigo-600 hover:underline">Registar o primeiro fornecedor</Link>}</div>}
    </div>
    <ConfirmDialog aberto={Boolean(fornecedorExcluir)} titulo="Excluir fornecedor?" descricao={`O fornecedor “${fornecedorExcluir?.nome || ""}” será removido.`} ocupada={Boolean(removendo)} onCancelar={() => !removendo && setFornecedorExcluir(null)} onConfirmar={remover}/>
  </main>;
}
