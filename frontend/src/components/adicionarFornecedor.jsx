...
export default function AdicionarFornecedor() {
  ...
  const guardar = async (event) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.erro || "Não foi possível criar o fornecedor.");
      navigate(location.state?.returnTo || "/fornecedor", { replace: true });
    } catch (error) {
  ...
  return (
    <FormularioFornecedor
      ...
      subtitulo="Cadastre um novo fornecedor para a sua empresa."
      form={form}
      mudar={mudar}
      erro={erro}
      salvando={salvando}
      guardar={guardar}
      cancelar={() => navigate(location.state?.returnTo || "/fornecedor")}
      textoBotao="Guardar fornecedor"
    />
  );
}
