...
export default function Register() {
  const [form, setForm] = useState({ nome: "", empresa_nome: "", email: "", senha: "" });
  ...
  const handleSubmit = async (event) => {
    ...
    try { const sessao = await register({ ...form, plano_id: planoId }); iniciarSessao(sessao); navigate(sessao.pagamentoNecessario ? "/assinatura" : "/painel", { replace: true }); }
    catch (error) { setErro(error.message); }
    finally { setLoading(false); }
  };

  return <main className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
    <form onSubmit={handleSubmit} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl space-y-4">
      ...
      <label className="block text-sm font-medium">Senha<input className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" type="password" minLength="6" value={form.senha} onChange={alterar("senha")} required /></label>
